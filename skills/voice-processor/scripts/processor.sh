#!/usr/bin/env bash
#
# Voice Processor v3 - With auto-formatting
# Processes incoming voice messages: transcribe + format + respond + cleanup
# Uses local Whisper with auto-formatting (FREE!)
#
...
WHISPER_FORMAT="/home/node/.openclaw/workspace/skills/local-whisper/scripts/transcribe_format.py"
...
MODEL_SIZE="${WHISPER_MODEL:-small}"  # small for best accuracy
...
# Transcribe using local Whisper with auto-formatting
transcribe_local() {
  local audio_file="$1"
  local transcript_file="$2"

  log "🎙️ Using LOCAL Whisper ($MODEL_SIZE) with auto-format..."

  if "$WHISPER_FORMAT" "$audio_file" "$MODEL_SIZE" "$LANGUAGE" > "$transcript_file" 2>&1 | tee -a "$LOG"; then
    log "✅ Local transcription with formatting successful"
    return 0
  else
...
# Transcribe using OpenAI API (no formatting)
transcribe_api() {
  local audio_file="$1"
  local transcript_file="$2"
  ...
  if "$WHISPER_API" "$audio_file" --out "$transcript_file" --lang "$LANGUAGE" 2>&1 | tee -a "$LOG"; then
...
process_voice() {
  ...
  log "Transcription mode: $TRANSCRIPTION_MODE | Model: $MODEL_SIZE | Lang: $LANGUAGE"

  # Transcribe
  local transcript_file="${TRANSCRIPTS_DIR}/${message_id}.txt"
  local timestamp=$(date '+%Y%m%d_%H%M%S')

  log "Transcribing with auto-formatting..."
