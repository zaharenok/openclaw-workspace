#!/usr/bin/env python3
"""
OpenClaw Webhook Server (HTTPS version)
    ...
    def do_GET(self):
        ...
        if self.path == '/':
            ...
            self.wfile.write(json.dumps({
                "status": "running",
                "service": "OpenClaw Webhook Server (HTTPS)",
...
def run_server():
    ...
    context.load_cert_chain('/etc/ssl/certs/ssl-cert-snakey.crt', '/etc/ssl/private/ssl-cert-snakey.key')

    httpd.socket = context.wrap_socket(httpd.socket)

    print(f"🚀 OpenClaw Webhook Server (HTTPS) starting on port {WEBHOOK_PORT}")
